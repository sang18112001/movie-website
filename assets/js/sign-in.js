...
const signIntoUp = document.querySelector('.sign-in-body .sign-converting span')
const signUptoIn = document.querySelector('.sign-up-body .sign-converting span')

signIntoUp.addEventListener('click', (event) => {
	console.log(signIntoUp)
...
signUptoIn.addEventListener('click', (event) => {
	console.log(signIntoUp)
	event.preventDefault()
	...
	signInBox.addEventListener('submit', (event) => {
		...
		async function modifyAPI() {
			...
			for (i = 0; i < valuesArr.length; i++) {
				if (valuesArr[i].email == your_account.email &&
					valuesArr[i].password == your_account.password
				) {
					check = true;
					valuesArr[i].checkSignIn = true
					alert("Sucess")
					window.location.assign(`index.html?uid=${keysArr[i]}`)
			...
			fetch(`https://fir-tutorial-32b97-default-rtdb.asia-southeast1.firebasedatabase.app/user/${keysArr[i]}.json`, {
				method: 'PUT',
				headers: {
	...
	signUpBox.addEventListener('submit', (event) => {
		...
		fetch('https://fir-tutorial-32b97-default-rtdb.asia-southeast1.firebasedatabase.app/user.json')
			...
			.then(accounts => {
				...
				if (!emails.includes(addNewAccount.email) && new_account.password === new_account.retype && new_account.checkLincence === "on") {
