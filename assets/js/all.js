...
(async function topRated() {
  ...
  for (i = 0; i < 8; i++) {
    const detailLink = !uid
      ? `sign-in.html`
      : `detailMovie.html?uid=${uid}&id=${movies[i].id}`;
    const movieElement = `
...
(async function genres() {
  ...
  genres_info.forEach((genre) => {
    const newLink = !uid
    ? `sign-in.html`
    : `typeOfMovies.html?uid=${uid}&type=popularity`;
  ...
  movies.forEach((movie) => {
    ...
    const movieOwl = `
            <div class="slider-item">
                <img class="border-cards image-banner opacity-image" src="${
                  IMG_PATH + img_path
                }" alt="">
            </div>
        `;
...
function show_movie_slider(movies) {
  ...
  movies.forEach((movie, index) => {
    const detailLink = !uid
      ? `sign-in.html`
      : `detailMovie.html?uid=${uid}&id=${movie.id}`;
    const movieElement = `
...
(function () {
  ...
  showAllButtons.forEach((button) => {
    const newLink = !uid
      ? `sign-in.html`
      : `typeOfMovies.html?uid=${uid}&type=${
          button.querySelector(".load-more").id
        }`;
    button.href = newLink;
  });
})();
